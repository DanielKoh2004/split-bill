...
// ═══════════════════════════════════════════════════════════
// Mock Data — Mamak Maju, KL, March 29 2026
// ═══════════════════════════════════════════════════════════
...
const MOCK_ITEMS: ReceiptItemDisplay[] = [
  { id: "1", name: "Nasi Lemak Ayam Goreng", quantity: 1, priceInCents: 1200 },
  { id: "2", name: "Maggi Goreng", quantity: 1, priceInCents: 800 },
  { id: "3", name: "Teh Tarik", quantity: 2, priceInCents: 700 },
  { id: "4", name: "Roti Canai", quantity: 2, priceInCents: 450 },
];

const MOCK_RECEIPT: ReceiptType = {
  items: MOCK_ITEMS.map((i) => ({
    id: i.id,
    name: i.name,
    priceInCents: i.priceInCents,
  })),
  subtotalInCents: 3150,
  taxInCents: 189,
  serviceChargeInCents: 315,
  grandTotalInCents: 3654,
};

const MERCHANT_NAME = "Mamak Maju";
const RECEIPT_DATE = "2026-03-29";
...
export default function GuestClaimView() {
  // ── State ───────────────────────────────────────────────
  ...
  const userTotal = useMemo(() => {
    ...
    try {
      ...
      // Build full claim list: guest claims + remaining shares go to "others"
      const allClaims = userClaims.flat();

      // Add "others" claims for remaining quantities
      ...
      const result = calculateSplit(MOCK_RECEIPT, allClaims);
  ...
  const breakdown = useMemo(() => {
    ...
    const weight =
      MOCK_RECEIPT.subtotalInCents > 0
        ? userTotal / MOCK_RECEIPT.grandTotalInCents
        : 0;
    const subtotal = Math.round(
      MOCK_RECEIPT.subtotalInCents * weight,
    );
    const tax = Math.round(MOCK_RECEIPT.taxInCents * weight);
    ...
    <div className="max-w-md mx-auto min-h-screen bg-slate-50 relative pb-32">
      ...
      <header className="px-5 pt-8 pb-4">
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            ...
            <p className="text-sm text-[#64748B]">{RECEIPT_DATE}</p>
          </div>
        </div>
        ...
        <div className="space-y-1 text-sm">
          <div className="flex justify-between">
            ...
            <span className="text-[#1E293B] font-medium">
              {formatRM(MOCK_RECEIPT.subtotalInCents)}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-[#64748B]">SST (6%)</span>
            <span className="text-[#1E293B] font-medium">
              {formatRM(MOCK_RECEIPT.taxInCents)}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-[#64748B]">Service Charge (10%)</span>
            <span className="text-[#1E293B] font-medium">
              {formatRM(MOCK_RECEIPT.serviceChargeInCents)}
            </span>
          </div>
          <div className="border-t border-slate-100 pt-2 mt-2 flex justify-between">
            ...
            <span className="font-bold text-[#1E293B]">
              {formatRM(MOCK_RECEIPT.grandTotalInCents)}
            </span>
          </div>
        </div>
      </div>
      ...
      {showModal && (
        ...
        >
          ...
          <div
            className="
              relative w-full max-w-md bg-white rounded-t-3xl p-6
          ...
          >
            ...
            <button
              onClick={handleCopy}
              className="
                w-full py-3.5 rounded-2xl bg-[#1E293B] text-white font-semibold
